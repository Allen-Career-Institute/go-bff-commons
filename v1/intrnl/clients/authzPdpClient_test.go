...
func Test_EnforceAbacs(t *testing.T) {
	ctrl, c, e, log := getTestingParams(t)
	...
	for i, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			_, err := cm.EnforceAbac(ctx, c, &types.ResourceAttributes{}, types.Action_ACTION_UNSPECIFIED)
...
func Test_GetUserAttributes(t *testing.T) {
	ctrl, c, e, log := getTestingParams(t)
	...
	for i, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			_, err := cm.GetUserAttributes(ctx, c, types.ResourceTypes_RESOURCE_UNSPECIFIED)
...
func Test_GetUserResources(t *testing.T) {
	ctrl, c, e, log := getTestingParams(t)
	...
	for i, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			_, err := cm.GetUserResources(ctx, c)
...
func Test_GetUserPolicies(t *testing.T) {
	ctrl, c, e, log := getTestingParams(t)
	...
	for i, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			_, err := cm.GetUserPolicies(ctx, c, "123")
			if err != nil && !reflect.DeepEqual(tc.expectedError.Error(), err.Error()) {
				t.Errorf("Testcase: %v FAILED, Expected: %v, Got: %v", i+1, tc.expectedError, err.Error())
			}
		})
